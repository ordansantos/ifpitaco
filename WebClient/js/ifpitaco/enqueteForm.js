...
ENQUETEFORM = (function(){
	...
	enqueteform.more = function (){
		if (enqueteform.qtd_opt == 2) $('#less').show();
		enqueteform.qtd_opt++;
		...
		if (enqueteform.qtd_opt == 5) $('#more').hide();
		
	}

	enqueteform.less = function (){
		
		if (enqueteform.qtd_opt == 5)  $('#more').show();
		...
		if (enqueteform.qtd_opt == 2) $('#less').hide();
	}
	
	enqueteform.resetNewEnquete = function(){
		
		$("#form_new_enquete")[0].reset();
		for (i = enqueteform.qtd_opt; i > 2; i--){
			$('#opt_'+i).remove();
			$('#l_'+i).remove();
		}
		enqueteform.qtd_opt = 2;
